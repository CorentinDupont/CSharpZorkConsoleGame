...
namespace TP_CS_ZORK.CONSOLE.utils
{
    static class GameInstance
    {
        ...
        public static Cell GetNextCell(string direction)
        {
            ...
            switch (direction){
                case "est":
                    return map.Single(c => c.PosX + 1 == _playerInstance.CurrentCell.PosX + 1 && c.PosY == _playerInstance.CurrentCell.PosY);
                    
                case "west":
                    return map.Single(c => c.PosX - 1 == _playerInstance.CurrentCell.PosX - 1 && c.PosY == _playerInstance.CurrentCell.PosY);

                case "north":
                    return map.Single(c => c.PosY + 1 == _playerInstance.CurrentCell.PosY + 1 && c.PosX == _playerInstance.CurrentCell.PosX);
